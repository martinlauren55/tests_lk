...
class Applicant(LoggedInUser):
    ...
    def set_citizenship(self, citizenship):
        ...
        browser.get(url)
        import sys
        print(browser.find_element_by_tag_name("body").text, file=sys.stderr)
        if citizenship == "Российская Федерация":
            ...
            try:
                ...
                try:
                    element = browser.find_element_by_xpath(f'//button[contains(.,"{text}")]')
                except NoSuchElementException:
                    #element = browser.find_element_by_xpath(f'//input[@value="{text}"][@type="submit"]')
                    element = browser.find_element_by_xpath(f'//div[contains(.,"{text}")]')
            element.click()
